...
<section class="container">
    <form action="{{ route('admin.characters.store', $character->slug) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-3">
            <label for="name" class="form-label">Name</label>
            <input type="text" class="form-control @error('name') is-invalid
                    @enderror" id="name" required name="name" value="{{ old('name', $character->name) }}">
            @error('name')
        ...
        <div class="row">
            ...
            <div class="col-2">
                <input id="type{{ $type->id }}" @if ($type->id == old('type_id', $character->type_id)) checked
                @endif
                ...
                <div class="accordion-item">
                    ...
                    <div id="collapse{{ $key }}" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                        <div class="accordion-body row">
                            ...
                            <div class="col-2">
                                <input id="item{{ $item->id }}" class="form-checkbox @error('item') is-invalid
                                                @enderror" type="checkbox" name="items[]" @if (in_array($item->id,
                                $character->items->pluck('id')->toArray())) checked @endif
                                value="{{ $item->id }}">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('description') is-invalid
                    @enderror" id="description" aria-describedby="emailHelp"
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('attack') is-invalid
                    @enderror" id="attack" required name="attack" value="{{ old('attack', $character->attack) }}">
            @error('attack')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('defence') is-invalid
                    @enderror" id="defence" required name="defence" value="{{ old('defence', $character->defence) }}">
            @error('defence')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('speed') is-invalid
                    @enderror" id="speed" required name="speed" value="{{ old('speed', $character->speed) }}">
            @error('speed')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('life') is-invalid
                    @enderror" id="life" required name="life" value="{{ old('life', $character->life) }}">
            @error('life')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
